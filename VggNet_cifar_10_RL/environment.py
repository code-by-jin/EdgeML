...
class Environment(object):
    ...
    def init_param_to_store(self):
        ...
        self.output_point = []
        self.times = []
        self.ondeviceLatency = []

        self.epoch = []
        self.bandwidth_epoch = []
        self.latency_epoch = []
        self.ondeviceLatency_epoch = []
        self.power_epoch = []
        self.accuracy_epoch = []
    ...
    def calculate_p(self, out_point, power, late):
        ...
        if self.power_u < power and self.late_u < late:
            reward = -  (4 / np.pi) * (np.arctan(delta_d)+ np.arctan(delta_p))
    ...
    def take_action(self, action, late_rl):
        ...
        for i in range(round_to_jump+1):
            ...
            self.ondeviceLatency.append(next_state[3])
        ...
        self.ondeviceLatency_epoch.append(average_state[3])
    ...
    def main(self, epoch):
        ...
        for i in range(epoch):
            if(i>400 and key1 == 0):
                os.system('wondershaper wlp2s0 2550 2550')
                key1 == 1
            if(i>600 and key2 == 0):
            ...
            """if(i>500 and key1 == 0):
                os.system('wondershaper wlp2s0 1750 1750')
                key1 == 1
            if(i>1000 and key2 == 0):
                os.system('wondershaper clear wlp2s0')
                key2 == 1
            if(i>1500 and key3 == 0):
                os.system('wondershaper wlp2s0 900 900')
                key3 == 1
        ...
        if plot:
            ...
            #0: non-offloading 1:offloading
            whetherOffloading = []
            for i in self.epoch:
                if(self.output_point_epoch[i-1]<1):
                    if(self.partition_point[i-1]>5):
                        whetherOffloading.append(0)
                    else:
                        whetherOffloading.append(1)
                    continue
                if(self.output_point_epoch[i-1]<2):
                    if(self.partition_point[i-1]>9):
                        whetherOffloading.append(0)
                    else:
                        whetherOffloading.append(1)
                    continue
                if(self.output_point_epoch[i-1]<3):
                    if(self.partition_point[i-1]>13):
                        whetherOffloading.append(0)
                    else:
                        whetherOffloading.append(1)
                    continue
                if(self.output_point_epoch[i-1]==3):
                    if(self.partition_point[i-1]>17):
                        whetherOffloading.append(0)
                    else:
                        whetherOffloading.append(1)
                    continue
            itemZip = dict(zip(self.step,whetherOffloading))
            with open('results/whetherOffloading.json', 'w', encoding='utf-8') as fs:
                json.dump(itemZip, fs)


            itemZip = dict(zip(self.step,self.ondeviceLatency_epoch))
            with open('results/ondeviceLatency_epoch.json', 'w', encoding='utf-8') as fs:
                json.dump(itemZip, fs)
            ...
            plot_data(self.epoch, whetherOffloading, title="whetherOffloading")
            plot_data(self.epoch, self.ondeviceLatency_epoch, title="ondeviceLatency_epoch")
